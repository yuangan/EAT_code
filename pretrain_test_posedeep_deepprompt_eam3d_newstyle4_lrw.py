...
from modules.prompt import Mapper, EmotionalDeformationTransformer
...
# lrw_img_name = 'HAPPENING_00046'
# lrw_img_name = 'HAVING_00024'
lrw_img_name = 'CANCER_00033'

# root_mead = '/data5/gy/mead'
root_lrw = '/data2/gy/lrw/'
poseimg_path = '/data2/gy/lrw/'
...
def build_model(config, device_ids=[0]):
    ...
    if torch.cuda.is_available():
        mapper.to(device_ids[0])

    return generator, kp_detector, audio2kptransformer, sidetuning, mapper


def prepare_test_data(img_path, audio_path, opt, use_otherimg=False):
    ...
    pose_gz = gzip.GzipFile(f'{poseimg_path}/poseimg/{asp}.npy.gz', 'r')
    ...
    name_len = min(mel_tensor.shape[1], poseimg.shape[0], deepfeature.shape[0], num_frames)
...
def load_ckpt(ckpt, kp_detector, generator, audio2kptransformer, sidetuning, mapper):
...
def test_mead(ckpt, part=0, save_dir=" "):
    with open("config/prompt_st_eam3d_tanh_mapper.yaml") as f:
        config = yaml.load(f, Loader=yaml.FullLoader)
    # temp_audio = audio_path
    # print(audio_path)
    cur_path = os.getcwd()
    generator, kp_detector, audio2kptransformer, sidetuning, mapper = build_model(config)
    load_ckpt(ckpt, kp_detector=kp_detector, generator=generator, audio2kptransformer=audio2kptransformer, sidetuning=sidetuning, mapper=mapper)
    ...
    all_wavs2 = []
    all_wavs2.append(root_lrw+f'lrw_wavs/{lrw_img_name}.wav')
    all_wavs2.sort()
    print(all_wavs2)

    img_names = ['CANCER_00033', 'ARRESTED_00015', 'EVENING_00035', 'ABOUT_00040']
    ...
    for name in img_names:
        for ind in tqdm(range(len(all_wavs2))):
            # img_path = f'/data5/gy/mead/images_evp_25/train/{id}_neu_1_001/0001.jpg'
            # img_path = glob.glob(f'{root_lrw}/lrw_images/{lrw_img_name}/0001.jpg')[0] # png/jpg
            img_path = glob.glob(f'{root_lrw}/lrw_images/{name}/0001.jpg')[0] # png/jpg
            ...
            os.makedirs(f"{log_dir}/{f_name[:-4]}", exist_ok=True)
            for idx, pred in enumerate(predictions_gen):
                imageio.imsave(f"{log_dir}/{f_name[:-4]}/{idx}.jpg", pred)
...
if __name__ == '__main__':
    ...
    if args.mode == 0:
        ...
        for ckpt in ckpt_paths:
            ...
            print(epoch)
            if epoch > 33: # and epoch < 316: # epoch %4==0
            # if epoch >= 518:
                nsp = name.split(' ')
                savefile = nsp[0]+'_'+nsp[1]
                # ckpt = ckpt.replace(' ', '\\ ')
                print(f'./{savefile}_{epoch}/')
                test_mead(f'./{ckpt}', args.part, save_dir=f'./result_mapper/{savefile}_{epoch}_mapper/')
