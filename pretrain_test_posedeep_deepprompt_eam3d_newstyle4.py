...
def prepare_test_data(img_path, audio_path, opt, use_otherimg=False):
    ...
    name_len = min(mel_tensor.shape[1], deepfeature.shape[0])
...
def test_mead(ckpt, part=0, save_dir=" "):
    ...
    for i in all_wavs:
        # print(ids[part]+'_'+os.path.basename(i)[:-4])
        if 'W015_neu_1_005' in os.path.basename(i)[:-4]:
            all_wavs2.append(i)
    all_wavs2.sort()
    print(all_wavs2)
    ...
    for iid in ids:
        for ind in tqdm(range(len(all_wavs2))):
            ...
            phoneme_path = ph_paths[ind]

            # phs = parse_phoneme_file(phoneme_path, mel_fps=25)
            # read in data
