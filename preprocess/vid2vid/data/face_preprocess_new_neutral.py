...
#         self.A_paths.extend(glob.glob('/data3/shared/MEAD/*/video/front/neutral/level_1/*.mp4'))
        ...
        self.A_paths = glob.glob('/data3/vox/vox/new_neutral_2/videos/*')
...
#         check_path_valid(self.A_paths, self.B_paths)
        self.output_path = '/data3/MEAD_cropped/extra/'
...
#         A, B, I, seq_idx = self.update_frame_idx(self.A_paths, index)
        ...
        Asp = A_path.split('/')
        B_path = f'/data3/vox/vox/new_neutral_2/lmk/{Asp[-1][:-4]}.json'
...
#         return return_list
    ...
    def video2img(self, path):
        vc = cv2.VideoCapture(path) #读入视频文件
        rval=vc.isOpened()
        outputpath = path.replace('videos','images')[:-4]
        c=0
        if not rval:
            f = open('./failed.txt', 'a+')
            f.write(path+'\n')
        ...
        while rval:   #循环读取视频帧
            ...
            if rval:
                ...
                cv2.imwrite(outputpath + '/' + str(c).zfill(4) + '.jpg', Bi) #存储为图像
            else:
                break
        vc.release()
