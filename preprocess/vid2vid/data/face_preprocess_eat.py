...
class FaceDataset(BaseDataset):
    def initialize(self, opt):
        ...
        self.A_paths = (glob.glob(f'{self.root}/*.mp4'))
        
        self.output_path = f'../cropped/'
    ...
    def __getitem__(self, index):
        ...
        B_path = f'../lmk_fps25/{Asp}.json'
        output = self.output_path + f'{Asp}.mp4'
        video = cv2.VideoCapture(A_path)
        B_width = int(video.get(cv2.CAP_PROP_FRAME_WIDTH))
        B_height = int(video.get(cv2.CAP_PROP_FRAME_HEIGHT))
        ...
        os.system(f'ffmpeg -y -loglevel error -i {A_path} -filter:v "crop={w}:{h}:{self.min_x}:{self.min_y}, scale={scale}" -crf 10 {output}')
