...
### read in poseimg with gzip to save disk memory and train quicker
### only 1 data
class FramesWavsDatasetMEL25VoxBoxQG2ImgPromptNovelStyle2LRW(Dataset):
    """
    Dataset of videos and wavs, each video can be represented as:
      - an image of concatenated frames
      - wavs
      - folder with all frames
    """
    def __init__(self, root_dir, frame_shape=(256, 256, 3), id_sampling=False, is_train=True, imgname=None,
                 random_seed=0, pairs_list=None, augmentation_params=None, syncnet_T=24):
        self.root_dir = root_dir
        self.videos = os.listdir(root_dir)
        self.frame_shape = tuple(frame_shape)
        ...
        if os.path.exists(os.path.join(root_dir, 'train')):
            ...
            if id_sampling:
                ...
                # train_videos_ori = os.listdir(os.path.join(root_dir, 'train'))
                train_videos_ori = os.listdir(os.path.join(root_dir, 'test'))
                train_videos = []
                for t in train_videos_ori:
                    # if 'W037' in t:
                    if 'W009_neu_1_001' in t:
                        train_videos.append(t)
            test_videos = os.listdir(os.path.join(root_dir, 'test'))
            # self.root_dir = os.path.join(self.root_dir, 'train' if is_train else 'test')
        ...
        if is_train:
            ...
            for emo in ['Neutral']:
                idlists = []
                # idlists.append(['INQUIRY_00010', emo])
                if imgname==None:
                    idlists.append(['ABOUT_00050', emo])
                else:
                    idlists.append([imgname, emo])
                idlists = idlists
                self.videos_lrw_emo[emo] = idlists
                self.videos_lrw_len[emo] = len(idlists)
                self.length += len(idlists) # mead only
            print(self.length)
        else:
            self.videos = test_videos
            self.length+=len(self.videos)
    ...
    def __getitem__(self, idx):
        if self.is_train:
            if  idx >= len(self.videos): #384
                return self.getitem_lrw_emo(idx)
                # return self.getitem_neu(idx%len(self.videos))
            else:
                return self.getitem_neu(idx)
        else:
    ...
    def getitem_neu(self, idx):
        while 1:
            ...
            # neutral frames
            video_name_neu = vsp[0]+'_neu_1_'+'001'
            path_neus = path.replace(video_name, video_name_neu)
            path_neu = random.choice(glob.glob(path_neus))
            source_latent = np.load(path_neu.replace('images', 'latent')+'.npy', allow_pickle=True)
            num_frames_source = source_latent[1]['yaw'].shape[0]
            ...
            # for k in out.keys():
            #     print(k)
            #     if isinstance(out[k], list) or isinstance(out[k], int) or isinstance(out[k], str):
            #         print(out[k])
            #     elif isinstance(out[k], dict):
            #         for j in out[k].keys():
            #             print(j, type(out[k][j]))
            #     else:
            #         print(out[k].shape)
            assert(0)
            return out
   
    def getitem_lrw_emo(self, index):
        while 1:
            if self.is_train and self.id_sampling:
                assert(0)
            else:
                ...
                # emo_name = self.emo_label_list[emo_r]
                emo_name = 'Neutral'
                r = np.random.choice(len(self.videos_lrw_emo[emo_name]), replace=True, size=1)[0]

                # iter done, clear lrw indexes
                name = self.videos_lrw_emo[emo_name][r][0]
                path = os.path.join(self.root_lrw, 'lrw_images', name)
                wave_path = f'{self.root_lrw}/lrw_wavs/{name}.wav'
            ...
            # print(out['drivinglmk'].shape)
            try:
                deep_path = f'{self.root_lrw}/lrw_df32/{name}.npy'
                deeps = np.load(deep_path)
            except:
                print(deep_path)
                continue
            ...
            ### poseimg after AntiAliasInterpolation2d: num_frames, 1, 64, 64
            fposeimg = gzip.GzipFile(f'{self.root_lrw}/poseimg/{name}.npy.gz', "r")
            poseimg = np.load(fposeimg)

            ### bboxs files extracted
            bboxs = np.load(f'{self.root_lrw}/lrw_bbox/{name}.npy', allow_pickle=True)
...
### read in poseimg with gzip to save disk memory and train quicker
### only 1 data
class FramesWavsDatasetMEL25VoxBoxQG2ImgPromptNovelStyle2(Dataset):
    """
    Dataset of videos and wavs, each video can be represented as:
      - an image of concatenated frames
      - wavs
    ...
    def __init__(self, root_dir, frame_shape=(256, 256, 3), id_sampling=False, is_train=True,
        ...
        if is_train:
            ...
            for emo in self.emo_label_list:
                idlists = []
                with open(f'{vox_path}/voxselect/{emo}.txt', 'r') as femolist:
                    idtxt = femolist.readlines()[:10000]
                    for i in idtxt:
                        idlists.append([i.split(' ')[0], emo])
                idlists = idlists
                self.videos_vox_emo[emo] = idlists
                # self.videos_vox_woemo.extend(idlists)
                # self.indexes_vox[emo] = 0
