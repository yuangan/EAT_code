...
def train_batch_prompt_mapper2(text, config, generator, discriminator, kp_detector, audio2kptransformer, mapper, sidetuning, checkpoint, log_dir, dataset, device_ids):
    train_params = config['train_params']
    ...
    # optimizer_kp_detector = torch.optim.Adam(kp_detector.parameters(), lr=train_params['lr_kp_detector'], betas=(0.5, 0.999))
    optimizer_audio2kptransformer = torch.optim.Adam(audio2kptransformer.parameters(), lr=train_params['lr_audio2kptransformer'], betas=(0.5, 0.999))
    optimizer_mapper = torch.optim.Adam(mapper.parameters(), lr=train_params['lr_mapper'], betas=(0.5, 0.999))
    # optimizer_sidetuning = torch.optim.Adam(sidetuning.parameters(), lr=train_params['lr_sidetuning'], betas=(0.5, 0.999))

    if checkpoint is not None:
        start_epoch = Logger.load_cpk_a2kp_prompt_sidetuning(checkpoint, generator, discriminator, kp_detector, audio2kptransformer, None, sidetuning,
                                      None, None, None, optimizer_audio2kptransformer, None, None)
                                    #   None, None, None, None, None, None)
                                    #   optimizer_audio2kptransformer, optimizer_emotionprompt, optimizer_sidetuning)
                                    #   optimizer_generator, optimizer_discriminator, None, None)
                                    #   optimizer_generator, optimizer_discriminator, optimizer_kp_detector, None)
    else:
        start_epoch = 0
    ...
    scheduler_audio2kptransformer = MultiStepLR(optimizer_audio2kptransformer, train_params['epoch_milestones'], gamma=0.1,
                                        last_epoch=-1)
    scheduler_mapper = MultiStepLR(optimizer_mapper, train_params['epoch_milestones'], gamma=0.1,
                                        last_epoch=-1)
    # scheduler_sidetuning = MultiStepLR(optimizer_sidetuning, train_params['epoch_milestones'], gamma=0.1,
                                        # last_epoch=-1)
                                        # last_epoch=-1 + start_epoch * (train_params['lr_kp_detector'] != 0))
    ...
    generator_full = GeneratorFullModelBatchDeepPromptSTEAM3DNewStyle2(text, kp_detector, audio2kptransformer, mapper, sidetuning, generator, discriminator, train_params, estimate_jacobian=config['model_params']['common_params']['estimate_jacobian'])
    ...
    with Logger(log_dir=log_dir, visualizer_params=config['visualizer_params'], checkpoint_freq=train_params['checkpoint_freq']) as logger:
        for epoch in trange(start_epoch, train_params['num_epochs']):
            ...
            logger.log_epoch(epoch, {'generator': generator,
                                    ...
                                    #  'optimizer_kp_detector': optimizer_kp_detector,
                                     'optimizer_audio2kptransformer': optimizer_audio2kptransformer,
                                     'optimizer_mapper': optimizer_mapper
                                    #  'optimizer_sidetuning': optimizer_sidetuning
                                     }, inp=x, out=generated, save_visualize=train_params['train_with_img'])

def train_batch_prompt_mapper3(text, config, generator, discriminator, kp_detector, audio2kptransformer, mapper, sidetuning, checkpoint, log_dir, dataset, device_ids):
    train_params = config['train_params']
    ...
    if checkpoint is not None:
        start_epoch = Logger.load_cpk_a2kp_prompt_sidetuning(checkpoint, generator, discriminator, kp_detector, audio2kptransformer, None, sidetuning,
                                      optimizer_generator, None, None, optimizer_audio2kptransformer, None, None)
                                    #   None, None, None, None, None, None)
                                    #   optimizer_audio2kptransformer, optimizer_emotionprompt, optimizer_sidetuning)
                                    #   optimizer_generator, optimizer_discriminator, None, None)
                                    #   optimizer_generator, optimizer_discriminator, optimizer_kp_detector, None)
    ...
    generator_full = GeneratorFullModelBatchDeepPromptSTEAM3DNewStyle3(text, kp_detector, audio2kptransformer, mapper, sidetuning, generator, discriminator, train_params, estimate_jacobian=config['model_params']['common_params']['estimate_jacobian'])
