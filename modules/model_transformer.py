...
class GeneratorFullModelBatchDeepPromptSTEAM3DNewStyle2(torch.nn.Module):
    """
    Merge all generator related updates into single model for better multi-gpu usage
    """

    def __init__(self, text, kp_extractor, audio2kptransformer, emotionprompt, sidetuning, generator, discriminator, train_params, estimate_jacobian=True):
        super(GeneratorFullModelBatchDeepPromptSTEAM3DNewStyle2, self).__init__()
        self.kp_extractor = kp_extractor
        self.kp_extractor.eval()
        self.audio2kptransformer = audio2kptransformer
        self.emotionprompt = emotionprompt
        ...
        if self.loss_weights['clip'] != 0:
            ...
            if torch.cuda.is_available():
                ...
                # self.text_token = self.text_token.cuda()
                self.clip_model.eval()
            self.text_features = self.get_emotoken_novelstyle(text)
    ...
    def forward(self, x, train_with_img=False):
        ...
        if self.loss_weights['latent'] != 0:
            # loss_values['latent_emo'] = self.loss_weights['latent'] * F.mse_loss(he_new_driving['exp'], he_driving['exp'].squeeze(0))
            # loss_values['latent_emo_5678'] = self.loss_weights['latent'] * torch.abs(he_new_driving['exp'][:, :, 15:27] - he_driving['exp'][:,:,15:27])
            loss_values['latent_emo_5678y'] = self.loss_weights['latent'] * F.mse_loss(he_new_driving['exp'].reshape(bbs, bs, 15, 3)[:,:, 5:8, 1],
                                                                                         he_driving['exp'].reshape(bbs, bs, 15, 3)[:,:, 5:8, 1])
    ...
    def get_emotoken_novelstyle(self, text):
        if text:
            text_novelstyle = text
        else:
            text_novelstyle = ['the man feel painful'
                        'A painful looking.',
                        'the person seems painful.',
                        'He or she seems to be painfully talking with Brows furrowed.',
                        ...
                        # 'Brows furrowed, eyes wide, lips tightened and pressed together',
                        'The person is talking with extreme pain.']
        ts4 = [text_novelstyle]
        text_features = []
        with torch.no_grad():
            for text in ts4:
...
class GeneratorFullModelBatchDeepPromptSTEAM3DNewStyle3(torch.nn.Module):
    """
    Merge all generator related updates into single model for better multi-gpu usage
    """

    def __init__(self, text, kp_extractor, audio2kptransformer, emotionprompt, sidetuning, generator, discriminator, train_params, estimate_jacobian=True):
        super(GeneratorFullModelBatchDeepPromptSTEAM3DNewStyle3, self).__init__()
        self.kp_extractor = kp_extractor
        self.kp_extractor.eval()
        self.audio2kptransformer = audio2kptransformer
        self.emotionprompt = emotionprompt
    ...
    def get_emotoken_novelstyle(self, text=None):
        if text:
            text_novelstyle=text
        else:
            text_novelstyle = ['the man feel painful'
