...
from modules.util import SameBlock2d, DownBlock2d, ResBlock3d, SPADEResnetBlock
...
from modules.audioencoder import EA3DModule
...
class OcclusionAwareSPADEGenerator(nn.Module):
    ...
    def forward(self, source_image, kp_driving, kp_source):
        ...
        # Transforming feature representation according to deformation and occlusion
        output_dict = {}
        output_dict['feature_3d'] = feature_3d # add for ladder generator
        if self.dense_motion_network is not None:
            dense_motion = self.dense_motion_network(feature=feature_3d, kp_driving=kp_driving,
                                                     kp_source=kp_source)
            output_dict['mask'] = dense_motion['mask']
            ...
            if 'occlusion_map' in dense_motion:
                ...
                occlusion_map = None
            deformation = dense_motion['deformation']
            output_dict['deformation'] = deformation # add for ladder generator
            out = self.deform_input(feature_3d, deformation)
        ...
        # Decoding part
        out, up_feat = self.decoder(out)

        output_dict["up_feat"] = up_feat
        output_dict["prediction"] = out

        return output_dict


class OcclusionAwareSPADEGeneratorEam(nn.Module):

    def __init__(self, image_channel, feature_channel, num_kp, block_expansion, max_features, num_down_blocks, reshape_channel, reshape_depth,
                 num_resblocks, estimate_occlusion_map=False, dense_motion_params=None, estimate_jacobian=False):
        super(OcclusionAwareSPADEGeneratorEam, self).__init__()
        ...
        self.ea3d = EA3DModule(style_dim=128, num_features=32)
    ...
    def forward(self, source_image, kp_driving, kp_source, prompt):
        out = self.first(source_image)
        for i in range(len(self.down_blocks)):
            out = self.down_blocks[i](out)
        out = self.second(out)
        ...
        # adain 3d 0
        feature_3d = self.ea3d(feature_3d, prompt)
        # Transforming feature representation according to deformation and occlusion
        output_dict = {}
        output_dict['feature_3d'] = feature_3d # add for ladder generator
        if self.dense_motion_network is not None:
