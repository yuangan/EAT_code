...
def make_coordinate_grid(spatial_size, type, device=0):
    d, h, w = spatial_size
    # x = torch.arange(w).to(device).type(type)
    # y = torch.arange(h).to(device).type(type)
    # z = torch.arange(d).to(device).type(type)
