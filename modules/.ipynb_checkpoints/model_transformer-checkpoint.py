...
class GeneratorFullModel(torch.nn.Module):
    ...
    def forward(self, x):
        # print(x['source'].shape) # 1 ,3, 256, 256
        kp_canonical = self.kp_extractor(x['source'], with_feature=True)     # {'value': value, 'jacobian': jacobian}   
        he_driving = self.audio2kptransformer(x, kp_canonical)           # {'yaw': yaw, 'pitch': pitch, 'roll': roll, 't': t, 'exp': exp}
        print(he_driving)
        assert(0)
        ...
        if (self.loss_weights['equivariance_value'] + self.loss_weights['equivariance_jacobian']) != 0:
            ...
            transformed_he_driving = self.audio2kptransformer(transformed_frame)
